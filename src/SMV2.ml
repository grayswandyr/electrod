...
module Make_SMV_LTL (At : Solver2.ATOMIC_PROPOSITION)
  : Solver2.LTL with module Atomic = At = struct
  module I = Solver2.LTL_from_Atomic(At) 
...
module Make_SMV_file_format (Ltl : Solver2.LTL)
  : Solver2.MODEL with type ltl = Ltl.t and type atomic = Ltl.Atomic.t = struct
  ...
  let pp_enum_decl elo vartype out atoms =
    ...
    let pp_one_decl atom =
      ...
      let may = Domain.may name elo.Elo2.domain |> TupleSet.to_seq in
  ...
  let make_script_file dir = function
    | Solver2.File filename -> filename (* script given on the command line *)
    | Solver2.Default default ->
  ...
  let analyze ~conversion_time ~cmd ~script ~keep_files
    ...
    let keep_or_remove_files scr smv =
      if keep_files then
        ...
        (match script with
          | Solver2.Default _ -> IO.File.remove_noerr scr
          | Solver2.File _ -> ());
      ...
      if String.suffix ~suf:"is true" @@ Gen.get_exn spec then
        ...
        let module P =
          SMV_trace_parser.Make(struct
            let base = Domain.musts ~with_univ_and_ident:false elo.Elo2.domain
          end)
        in
        let trace =
          spec
