...
let rec remove_always_from_invar (Elo2.Fml { node; _} as fml) =
  let open Elo2 in
  match node with
    | LUn (G, subfml) -> subfml
    | LBin (fml1, And, fml2) ->
...
let add_always_to_invar (Elo2.Fml { node; _} as fml)  =
  let open Elo2 in
  match node with
    | LUn (G, _) -> fml
    | _ -> lunary always fml 
...
let is_const (elo : Elo2.t) (name : Name.t) =
  assert (Domain.mem name elo.Elo2.domain);
  Domain.get_exn name elo.Elo2.domain |> Relation.is_const

class ['self] computer (elo : Elo2.t) = object (_ : 'self)
  inherit ['self] Elo2.fold

  method build_fml () f' _ = f' 

  method build_Run () blk' =
    List.fold_left
      max_color_wiwt
      Static_prop
      blk'

  method build_Check () blk' =
    List.fold_left
      max_color_wiwt
      Static_prop
      blk'
  ...
  method build_Let () __bs' __block'= assert false (* SIMPLIFIED *)
