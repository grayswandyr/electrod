...
module Make (Ltl : Solver.LTL) = struct
  ...
  let eligible_pairs ((tuple, r_sup, s_sup) : Tuple.t * TS.t * TS.t) :
    ...
    let open Iter in
    let r_sup_seq = TS.to_seq r_sup in
    ...
    object (self : 'self)
      ...
      *)
      ...
      method build_In subst r (__s : G.exp) r' s' =
        ...
        +&& lazy
          (wedge ~range:(TS.to_seq may) (fun bs ->
                 lazy (r' bs @=> lazy (s' bs))))
      ...
      method build_Over (subst : stack) __r s r' s' tuple =
        ...
        let mustpart =
          wedge ~range:(TS.to_seq must) (fun t ->
                lazy
      ...
      method build_REq subst r s r' s' =
        ...
        +&& lazy
          (wedge ~range:(TS.to_seq s_bounds.must) (fun t -> lazy (r' t)))
        +&& lazy
          (wedge ~range:(TS.to_seq inter) (fun bs ->
                 lazy (r' bs @<=> s' bs)))
        +&& lazy
          (wedge
             ~range:(TS.to_seq @@ TS.diff r_bounds.may inter)
             (fun bs -> lazy (r' bs @=> lazy (s' bs))))
        +&& lazy
          (wedge
             ~range:(TS.to_seq @@ TS.diff s_bounds.may inter)
      ...
      method build_RTClos subst r _ tuple = (* FIXME *)
        ...
        let tuple_in_var_iden =
          ...
          then 
            env#get_set_names
            |> List.map 
                 (fun name -> self#visit_exp subst (G.name ~ar:1 name) (Tuple.tuple1 tuple0)) 
            |> disj
          else false_ 
        in
      ...
      method build_TClos subst r __r' tuple =
        ...
        Msg.debug (fun m ->
              m "TC formula: @[<h2> %a@]" (Fmtc.hbox2 Ltl.pp) fml) ;
        fml
