...
let rec convert_fml stack ({ prim_fml; _ } : (Ast.var, Ast.ident) Gen_goal.fml)
  ...
  | False -> E.false_
  | RComp (e1, comp, e2) ->
      E.rcomp (convert_exp stack e1) (convert_comp_op comp)
        (convert_exp stack e2)
  | IComp (e1, comp, e2) ->
      E.icomp (convert_iexp stack e1) (convert_icomp_op comp)
        (convert_iexp stack e2)
  | LUn (op, f) -> E.lunary (convert_lunop op) (convert_fml stack f)
  | LBin (f1, op, f2) ->
      E.lbinary (convert_fml stack f1) (convert_lbinop op)
        (convert_fml stack f2)
  | FIte (c, t, e) ->
      E.fite (convert_fml stack c) (convert_fml stack t) (convert_fml stack e)
  | Block fmls -> E.block @@ convert_block stack fmls
...
and convert_exp stack
  ...
  | Univ -> E.univ
  | Iden -> E.iden
  | Ident (Var v) -> E.var ~ar @@ get_var v stack
  | Ident (Name n) -> E.name ~ar n
  | RUn (op, e) -> E.runary ~ar (convert_runop op) (convert_exp stack e)
  | RBin (e1, op, e2) ->
      E.rbinary ~ar (convert_exp stack e1) (convert_rbinop op)
        (convert_exp stack e2)
  | RIte (c, t, e) ->
      E.rite ~ar (convert_fml stack c) (convert_exp stack t)
        (convert_exp stack e)
...
let convert (ast : Ast.t) =
  ...
  E.make ast.file ast.domain ast.instance ast.sym invariants goal
