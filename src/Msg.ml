...
module LoggingSetup = struct
  ...
  let electrod_src =
    Logs.Src.create "electrod.onera.fr" ~doc:"logs electrod events"
...
let code num = Printf.sprintf "%03d" num
...
module Fatal = struct
  let lexical args =
    err @@ fun m ->
    args @@ fun infile lexbuf msg ->
    let loc =
      Location.from_positions
        (Lexing.lexeme_start_p lexbuf)
        (Lexing.lexeme_end_p lexbuf)
    in
    m ~header:(code 1) "%a%a: lexical error: %s"
      (option @@ (colon **> string))
      infile Location.pp loc msg

  let syntax args =
    err @@ fun m ->
    args @@ fun file lexbuf ->
    let loc =
      Location.from_positions
        (Lexing.lexeme_start_p lexbuf)
        (Lexing.lexeme_end_p lexbuf)
  ...
  let wrong_suffix args =
    err @@ fun m ->
    args @@ fun infile id ->
    let name, loc = (Raw_ident.basename id, Raw_ident.location id) in
    m ~header:(code 3)
      "%a%a: syntax error: %S is not a valid suffix for indexed identifiers%a"
      (option @@ (colon **> string))
      infile Location.pp loc name (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let different_prefixes args =
    err @@ fun m ->
    args @@ fun infile first last ->
    let first_loc = Raw_ident.location first in
    let last_loc = Raw_ident.location last in
    m ~header:(code 4) "%a%S (%a) and %S (%a) have different_prefixes%a"
      (option @@ (sp **> colon **> string))
      infile (Raw_ident.basename first) Location.pp first_loc
      (Raw_ident.basename last) Location.pp last_loc (hardline **< Extract.pp)
      (Extract.extract infile (Location.span (first_loc, last_loc)))

  let not_an_interval args =
    err @@ fun m ->
    args @@ fun infile first last ->
    let first_loc = Raw_ident.location first in
    let last_loc = Raw_ident.location last in
    m ~header:(code 5)
      "%a%S (%a) and %S (%a) do not define a valid (increasing) range of \
       atoms%a"
      (option @@ (sp **> colon **> string))
      infile (Raw_ident.basename first) Location.pp first_loc
      (Raw_ident.basename last) Location.pp last_loc (hardline **< Extract.pp)
  ...
  let rel_name_already_used args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 6) "%a%a: identifier %S already used%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let undeclared_id args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 7) "%a%a: identifier %S unknown%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let should_denote_a_constant_set args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 8) "%a%a: %S does not denote a constant set%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let incompatible_arities args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 9) "%a%a: inconsistent arities used in some tuples for %S%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let undeclared_atoms args =
    err @@ fun m ->
    args @@ fun infile loc absent ->
    m ~header:(code 10) "%a%a: atom(s) not declared in 'univ': %a%a"
      (option @@ (colon **> string))
      infile Location.pp loc
      (vbox2 @@ list ~sep:sp Atom.pp)
      absent (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let inf_not_in_sup args =
    err @@ fun m ->
    args @@ fun infile id pp_bound inf sup ->
    let loc = Raw_ident.location id in
    m ~header:(code 11)
      "%a%a: lower bound of %S is not included in upper bound@\n\
       ...
       upper bound = %a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) Extract.pp
      (Extract.extract infile loc)
      (box2 @@ pp_bound) inf (box2 @@ pp_bound) sup

  let inexact_ref_used_in_exact_scope args =
    err @@ fun m ->
    args @@ fun infile id ref_id ->
    let loc = Raw_ident.location id in
    m ~header:(code 12)
      "%a%a: %S is declared as exact but refers to relation %S which has \
       inexact bounds%a"
      (option @@ (colon **> string))
  ...
  let arity_error args =
    err @@ fun m ->
    args @@ fun infile exp msg ->
    let loc = exp.Gen_goal.exp_loc in
    m ~header:(code 13) "%a%a: %s%a"
      (option @@ (colon **> string))
      infile Location.pp loc msg (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let init_and_fby_incompatible_arities args =
    err @@ fun m ->
    args @@ fun infile id init fby ->
    let loc = Raw_ident.location id in
    m ~header:(code 14)
      "%a%a: inconsistent arities used in the 'init' (%d) and 'then' (%d) \
       parts for %S%a"
      (option @@ (colon **> string))
      infile Location.pp loc init fby (Raw_ident.basename id)
      (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let cannot_decide_arity args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 15)
  ...
  let specified_computed_arities_discrepancy args =
    err @@ fun m ->
    args @@ fun infile id specified_arity computed_arity ->
    let loc = Raw_ident.location id in
    m ~header:(code 16)
      "%a%a: discrepancy between the specified (%d) and inferred (%d) arities \
       for %S%a"
      (option @@ (colon **> string))
      infile Location.pp loc specified_arity computed_arity
      (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let instance_is_var args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 17)
  ...
  let instance_not_in_scope args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 18)
      "%a%a: the value given for %S in the instance does not comply with the \
       declared scope%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let instance_already_declared args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 19) "%a%a: instance %S already declared%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let syntax_error_paragraphs args =
    err @@ fun m ->
    args @@ fun infile msg ->
    m ~header:(code 20) "%a%s" (option @@ (sp **> colon **> string)) infile msg

  let symmetry_wrongly_defined args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 21)
      ...
      (option @@ (colon **> string))
      infile Location.pp loc (hardline **< Extract.pp)
      (Extract.extract infile loc)

  let solver_failed args =
    err @@ fun m ->
    args @@ fun solver scr smv errcode erroutput ->
    m ~header:(code 22)
      "%s@ failed@ with@ error@ code@ %d@ and@ error@ output:@\n\
       %s@\n\
       Script@ and@ model@ files@ kept@ at@ %S@ and@ %S,@ remember@ to@ \
       remove@ them."
      solver errcode erroutput scr smv
  ...
  let multiplicity_only_in_a_sup args =
    err @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 25)
...
module Warn = struct
  let univ_duplicate_atoms args =
    warn @@ fun m ->
    args @@ fun __infile written actual ->
    let fmt = Fmtc.box2 @@ Atom.pp_list in
    m ~header:(code 1)
      "univ contains duplicate atoms...:@ %a@;\
       ...ignoring them and continuing with:@ %a" fmt written fmt actual

  let empty_scope_declared args =
    warn @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 2) "%a%a: %s is always empty@\n%a"
      (option @@ (colon **> string))
      infile Location.pp loc (Raw_ident.basename id) Extract.pp
      (Extract.extract infile loc)

  let duplicate_elements args =
    warn @@ fun m ->
    args @@ fun infile id bound_kind pp_bound bound ->
    let loc = Raw_ident.location id in
    ...
    m ~header:(code 3)
      ...
      (option @@ (colon **> string))
      infile Location.pp loc (pp_inf_sup bound_kind) (Raw_ident.basename id)
      Extract.pp
      (Extract.extract infile loc)
      (box2 @@ pp_bound) bound

  let disj_with_only_one_variable args =
    warn @@ fun m ->
    args @@ fun infile id ->
    let loc = Raw_ident.location id in
    m ~header:(code 4)
  ...
  let met_spurious_variable args =
    warn @@ fun m ->
    args @@ fun var ->
    m ~header:(code 5) "Spurious variable(s) in the SMV trace: %S" var
end
