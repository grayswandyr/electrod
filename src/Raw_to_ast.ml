...
let split_indexed_id infile id =
  ...
  | Some (left, right) ->
      let rightnum =
        ...
        with Failure _ -> Msg.Fatal.wrong_suffix (fun args -> args infile id)
      in
      (left, rightnum)
...
let compute_univ infile raw_univ =
  ...
  let atoms =
    flat_map
      (function
        | UIntvl intvl -> interval_to_atoms infile intvl
        | UPlain id -> [ Atom.of_raw_ident id ])
      raw_univ
  in
...
let compute_scope infile domain id = function
  ...
  | SInexact (raw_inf, Some function_type, raw_sup) -> (
      ...
      if not @@ TS.is_empty inf then
        ...
        | `Lone -> Scope.(fun s -> inexact @@ partial_function 0 s) sup
        | `One -> Scope.(fun s -> inexact @@ total_function 0 s) sup)
  ...
  | SInexact (raw_inf, None, BProd (rb1, Some function_type, rb2)) -> (
      ...
      if not @@ TS.is_empty inf then
        ...
        match function_type with
        | `Lone -> Scope.(fun s -> inexact @@ partial_function dom_ar s) sup
        | `One -> Scope.(fun s -> inexact @@ total_function dom_ar s) sup)
...
let compute_decl infile domain = function
  ...
  | DVar (id, specified_arity, init, fby) ->
      ...
      let computed_arity =
        match Option.map Scope.inferred_arity fby_scope with
...
let compute_instances domain (pb : Raw.raw_problem) =
  ...
  List.fold_left
    (fun acc asgn ->
      compute_assignment asgn |> fun (n, ts) ->
...
let refine_identifiers raw_pb =
  ...
  and walk_prim_fml ctx = function
    ...
    | True -> (ctx, true_)
    | False -> (ctx, false_)
    | Block b -> (ctx, Pair.snd_map block (walk_block ctx b))
    | LUn (op, fml) -> (ctx, Pair.snd_map (lunary op) (walk_fml ctx fml))
    | LBin (f1, op, f2) ->
...
let compute_arities elo =
  ...
  and walk_prim_fml ctx = function
    ...
    | RComp (e1, op, e2) ->
        ...
        let ar2 = e2'.arity in
        if
          (not @@ Option.equal ( = ) ar1 ar2)
          && Option.is_some ar1 && Option.is_some ar2
        then
          Msg.Fatal.arity_error (fun args ->
              args elo.Ast.file e2
                (Fmtc.str "arity of %s (%a) incompatible with that of %s (%a)"
                   (str_exp e1)
    ...
    | IComp (e1, op, e2) ->
        ...
        icomp e1' op e2'
    | LUn (op, fml) -> lunary op @@ walk_fml ctx fml
    | LBin (f1, op, f2) -> lbinary (walk_fml ctx f1) op (walk_fml ctx f2)
    | Quant (q, sbs, blk) ->
    ...
    | FIte (c, t, e) -> fite (walk_fml ctx c) (walk_fml ctx t) (walk_fml ctx e)
    | Block blk -> block @@ walk_block ctx blk
  ...
  and walk_exp ctx exp =
    ...
    | Ok exp' -> exp'
    | Error msg -> Msg.Fatal.arity_error (fun args -> args elo.Ast.file exp msg)
  ...
  and walk_prim_exp ctx exp =
    ...
    | None_ -> return_exp exp None none
    | Univ ->
    ...
    | RBin (e1, op, e2) -> (
        ...
        | Union ->
            Result.fail
              (Fmtc.str "incompatible arities between %s and %s" (str_exp e1')
                 (str_exp e2'))
        ...
        | Over when Option.equal ( = ) ar1 ar2 ->
            if Option.compare CCInt.compare ar1 (Some 1) <= 0 then
              Result.fail (Fmtc.str "arity of %s is < 2" (str_exp e1'))
        ...
        | LProj when Option.equal ( = ) ar1 (Some 1) ->
            return_exp exp ar2 @@ rbinary e1' op e2'
        | LProj -> Result.fail "left projection should be on a set"
        | RProj when Option.is_none ar2 ->
            return_exp exp None @@ rbinary e1' op e2'
        | RProj when Option.equal ( = ) ar2 (Some 1) ->
            return_exp exp ar1 @@ rbinary e1' op e2'
        | RProj -> Result.fail "right projection should be on a set"
        | Prod -> (
            match (ar1, ar2) with
            | Some a1, Some a2 ->
    ...
    | RIte (c, t, e) -> (
        ...
        | Some a, None | None, Some a -> return_exp exp (Some a) (rite c' t' e')
        | None, None -> return_exp exp None (rite c' t' e')
        | Some _, Some _ ->
            Result.fail
  ...
  let walk_goal ctx = function
    | Run (fmls, expec) -> run (List.map (walk_fml ctx) fmls) expec
  in
  Ast.
    {
      elo with
