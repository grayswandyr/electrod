...
module Make_SMV_file_format (Ltl : Solver.LTL)
  ...
  let make ~elo ~rigid ~flexible ~invariant ~property =
    { elo; rigid; flexible; invariant = Sequence.rev invariant; property }

  let pp_decl out atomic =
    Fmtc.pf out "%a : boolean;" Ltl.Atomic.pp atomic


  let pp ?(margin = 80) out { elo; rigid; flexible; invariant; property } =
    let open Fmtc in
