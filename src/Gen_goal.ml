...
and quant = All | Some_ | No | One | Lone
...
(* previous *)
and comp_op = In | NotIn | REq | RNEq
and icomp_op = IEq | INEq | Lt | Lte | Gt | Gte
...
and pp_prim_fml pp_v pp_i out =
  let open Fmtc in
  function
  | True -> (kwd_styled pf) out "true"
  | False -> (kwd_styled pf) out "false"
  | Qual (q, exp) ->
      pf out "@[<2>(%a@ %a)@]" pp_rqualify q (pp_exp pp_v pp_i) exp
  | RComp (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_exp pp_v pp_i) e1 pp_comp_op op
        (pp_exp pp_v pp_i) e2
  | IComp (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_iexp pp_v pp_i) e1 pp_icomp_op op
        (pp_iexp pp_v pp_i) e2
  | LUn (op, fml) -> pf out "@[<2>(%a@ %a)@]" pp_lunop op (pp_fml pp_v pp_i) fml
  | LBin (f1, op, f2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_fml pp_v pp_i) f1 pp_lbinop op
        (pp_fml pp_v pp_i) f2
  | Quant (q, decls, blk) ->
      pf out "@[<2>(%a %a@ %a)@]" pp_quant q
        (list ~sep:(sp **> comma) @@ pp_sim_binding pp_v pp_i)
        decls (pp_block pp_v pp_i) blk
  | Let (bindings, blk) ->
      pf out "%a %a@ %a" (kwd_styled string) "let"
        (list ~sep:(sp **> comma) @@ pp_binding ~sep:equal pp_v pp_i)
        bindings (pp_block pp_v pp_i) blk
  | FIte (c, t, e) ->
      (* pf out "@[<hv2>(%a@ @[implies %a@]@ @[else %a@])@]" *)
      pf out "@[<hv>(%a) %a@;<1 2>@[(%a@])@;%a@;<1 2>@[(%a@])@]"
        (pp_fml pp_v pp_i) c (kwd_styled string) "implies" (pp_fml pp_v pp_i) t
        (kwd_styled string) "else" (pp_fml pp_v pp_i) e
  | Block fmls -> pp_block pp_v pp_i out fmls
...
and pp_prim_exp pp_v pp_i out =
  ...
  | Univ -> (styled Name.style pf) out "univ"
  | Iden -> (styled Name.style pf) out "iden"
  | Ident id -> pf out "%a" pp_i id
  | RUn (op, e) -> pf out "@[<2>(%a%a)@]" pp_runop op (pp_exp pp_v pp_i) e
  | RBin (e1, Join, e2) ->
      (* special one for join *)
      pf out "@[<2>(%a.%a)@]" (pp_exp pp_v pp_i) e1 (pp_exp pp_v pp_i) e2
  | RBin (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_exp pp_v pp_i) e1 pp_rbinop op
        (pp_exp pp_v pp_i) e2
  | RIte (c, t, e) ->
      pf out "@[<hv>%a %a@;<1 2>@[%a@]@;%a@;<1 2>@[%a@]@]" (pp_fml pp_v pp_i) c
        (kwd_styled string) "implies" (pp_exp pp_v pp_i) t (kwd_styled string)
        "else" (pp_exp pp_v pp_i) e
  | BoxJoin (exp, args) ->
      pf out "@[<2>(%a%a)@]" (pp_exp pp_v pp_i) exp
...
and pp_prim_iexp pp_v pp_i out =
  ...
  function
  | Num n -> pf out "%d" n
  | Card exp -> pf out "@[<2>(#%a)@]" (pp_exp pp_v pp_i) exp
  | IUn (op, iexp) ->
