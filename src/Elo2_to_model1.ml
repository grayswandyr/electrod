...
module Make
    (Ltl : Solver2.LTL)
    (ConvertFormulas : Elo2_to_LTL_intf2.S
...
struct
  ...
  let split_invar_noninvar_fmls elo blk =
    let open Invar_computation2 in
    let (invf, tmp_restf) =
      List.partition_map
        (fun fml ->
           ...
           (* Msg.debug (fun m ->
              ...
              Elo2.pp_fml fml Invar_computation2.pp color); *)
           match color with
  ...
  let run elo =
    ...
    (* handling the goal *)
    let goal_blk = match elo.goal with Elo2.Run g -> g in
    ...
    in
    (* Msg.debug (fun m ->
       m "Detected init : %a" Elo2.pp_block detected_inits); *)

    (* Msg.debug (fun m ->
       m "Detected invariants : %a" Elo2.pp_block detected_invars); *)

    (* Msg.debug (fun m ->
       m "Detected trans : %a" Elo2.pp_block detected_trans); *)

    let spec_fml = dualise_fmls general_fmls in
    (* Msg.debug (fun m -> m "Elo2 property : %a" Elo2.pp_fml spec_fml); *)
    ...
    let invars =
      translate_formulas @@ List.append detected_invars elo.Elo2.invariants
    in
