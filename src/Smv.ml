    ...
    *)
    ...
    let pp ?(next_is_X = true) bitwidth variables upper out f =
      ...
      and pp_term upper out (t : term) =
        ...
        | Bin (t1, Div, t2) ->
            pf out "%s(%a, %a)" divide_m (pp_term 0) t1 (pp_term 0) t2
        | Bin (t1, Rem, t2) ->
            pf out "%s(%a, %a)" rem_m (pp_term 0) t1 (pp_term 0) t2
        | Bin (t1, Lshift, t2) ->
...
module Make_SMV_file_format (Ltl : Solver.LTL) :
  ...
  let pp_count_variables ?(margin = 80) out
    ...
    let two_bw_minus_1 =
      if bitwidth <= 0 then 0 else Int.(2 ** (bitwidth - 1))
    in
    ...
    pf out
      ...
      \ (case\\\n\
      \   a = 0sd%d_0 : 0sd%d_0;\\\n\
      \   b = 0sd%d_0 & a < 0sd%d_0 : -0sd%d_1;\\\n\
      \   b = 0sd%d_0 & a > 0sd%d_0 : 0sd%d_1;\\\n\
      \   a = -0sd%d_%d & b = -0sd%d_1 : a;\\\n\
      \   TRUE : a / b;\\\n\
      \ esac)     \n\
       #define %s(a,b)\\\n\
      \ (case\\\n\
      \   a = 0sd%d_0 : 0sd%d_0;\\\n\
      \   b = 0sd%d_0 : a;\\\n\
      \   a = -0sd%d_%d & b = -0sd%d_1 : 0sd%d_0;\\\n\
      \   TRUE : a mod b;\\\n\
      \ esac)@\n"
      divide_m bitwidth bitwidth bitwidth bitwidth bitwidth bitwidth bitwidth
      bitwidth bitwidth two_bw_minus_1 bitwidth rem_m bitwidth bitwidth bitwidth
      bitwidth two_bw_minus_1 bitwidth bitwidth;
    (* INIT *)
