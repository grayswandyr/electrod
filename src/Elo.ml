...
type ('fml, 'exp, 'iexp) ofml =
  ...
  | Quant of quant
             * ((bool [@opaque]) * (int [@opaque]) * 'exp) (* int : number of bound variables (>= 1); if = 1 then disj must be false *)
             * 'fml list
  | FIte of 'fml * 'fml * 'fml 
  | Block of 'fml list
...
class virtual ['self] fold = object (self : 'self)
  ...
  method visit_'fml env (Fml { node; _}) =
    (self # visit_ofml env node)

  method visit_'iexp env (Iexp { node; _}) =
    (self # visit_oiexp env node)
...
let pp_prim_oexp stacked pp_fml pp_exp out = 
  ...
  function
    ...
    | Compr (decls, blk) ->
        let nbvars = 
          List.fold_left (fun acc (_, nb, _) -> acc + nb) 0 decls in
        pf out "%a"
          (braces_ @@
           pair ~sep:sp
