...
let pp_ofml stacked pp_fml pp_exp pp_iexp out =
  ...
  | RComp (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_exp stacked) e1 pp_comp_op op
        (pp_exp stacked) e2
  | IComp (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_iexp stacked) e1 pp_icomp_op op
        (pp_iexp stacked) e2
  | LUn (op, fml) -> pf out "@[<2>(%a@ %a)@]" pp_lunop op (pp_fml stacked) fml
  | LBin (f1, op, f2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_fml stacked) f1 pp_lbinop op
        (pp_fml stacked) f2
  ...
  | FIte (c, t, e) ->
      pf out "@[<hv>(%a) %a@;<1 2>@[(%a@])@;%a@;<1 2>@[(%a@])@]"
        (pp_fml stacked) c (kwd_styled string) "implies" (pp_fml stacked) t
...
let pp_prim_oexp stacked pp_fml pp_exp pp_iexp out =
  ...
  | Name id -> pf out "%a" Name.pp id
  | Var v ->
      assert (v <= stacked);
      pp_var out (stacked - v)
  | RUn (op, e) -> pf out "@[<2>(%a%a)@]" pp_runop op (pp_exp stacked) e
  | RBin (e1, Join, e2) ->
      (* special one for join *)
      pf out "@[<2>(%a.%a)@]" (pp_exp stacked) e1 (pp_exp stacked) e2
  | RBin (e1, op, e2) ->
      pf out "@[<2>(%a@ %a@ %a)@]" (pp_exp stacked) e1 pp_rbinop op
        (pp_exp stacked) e2
  | RIte (c, t, e) ->
      pf out "@[<hv>%a %a@;<1 2>@[%a@]@;%a@;<1 2>@[%a@]@]" (pp_fml stacked) c
        (kwd_styled string) "implies" (pp_exp stacked) t (kwd_styled string)
        "else" (pp_exp stacked) e
  | Compr (decls, blk) ->
...
let pp out { domain; instance; invariants; goal; _ } =
  let open Fmtc in
  pf out "%a@\n%a@\n%a@\n%a" Domain.pp domain Instance.pp instance
    (vbox2 @@ styled `Bold
    @@ (const string "invariant" **< cut **< (Fmtc.list @@ pp_fml 0)))
    invariants (vbox @@ pp_goal) goal
