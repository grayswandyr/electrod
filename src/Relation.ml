...
let equal rel1 rel2 =
  ...
  | Const c1, Const c2 ->
      Name.equal c1.name c2.name && c1.arity = c2.arity
...
let pp ?(print_name = true) out rel =
  ...
  in
  match rel with
  | Const { name; scope; arity } -> pp_def "const" name scope None arity
  | Var { name; scope; fby; arity } -> pp_def "var" name scope fby arity
